...
app.post('/bot-webhook', async (req, res) => {
  try {
    ...
    const isReset = text === '/resetadmin' || text === '/claimadmin' || text.includes('force');

    if (isStart || isReset) {
      if (data.adminChatId && data.adminChatId !== chatId && !isReset) {
        await bot.sendMessage(chatId, `⚠️ Bot is already bound to admin ID: ${data.adminChatId}.\n\nTo reset/claim admin access for your chat ID (${chatId}), send: /resetadmin`);
...
/setadmin <chatId> — Transfer/set admin Chat ID (or /setadmin me)
/resetadmin — Reset admin to current chat ID
...
/addbank Rahul Kumar|1234567890|SBIN0001234|SBI|rahul@upi`
    ...
    if (data.adminChatId && chatId !== data.adminChatId) {
      await bot.sendMessage(chatId, `❌ Unauthorized chat ID (${chatId}). Send /resetadmin to claim admin rights.`);
      return res.sendStatus(200);
    }
    ...
    if (text.startsWith('/setadmin ') || text.startsWith('/changeadmin ')) {
      ...
      if (!targetChatId) {
        await bot.sendMessage(chatId, '❌ Format: /setadmin <chatId> or /setadmin me\nExample: /setadmin 123456789');
        return res.sendStatus(200);
      }
      ...
      data._skipOverrideMerge = true;
      await saveData(data);
      await bot.sendMessage(chatId, `✅ Admin Chat ID updated to: \`${targetChatId}\``, { parse_mode: 'Markdown' });
      if (String(targetChatId) !== String(chatId)) {
        bot.sendMessage(targetChatId, `👑 You have been set as the Admin for IUKPay Controller! Send /start to access the panel.`).catch(()=>{});
      }
      return res.sendStatus(200);
    }
